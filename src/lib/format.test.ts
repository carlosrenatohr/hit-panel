...
import { foldAccents, fmtDate, toLocalDate, altCurrencyTotal, waPhone } from './format'
...
describe('altCurrencyTotal', () => {
  it('shows just the córdoba symbol+amount when the working currency is USD', () => {
    const out = altCurrencyTotal(100, 'USD', 37)
    expect(out).toBe('C$3,700.00')
  })

  it('shows just the USD symbol+amount when the working currency is NIO', () => {
