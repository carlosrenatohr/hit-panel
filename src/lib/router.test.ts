...
describe('router', () => {
  ...
  it('carries a ?cliente= seed into /envios and parses it back', () => {
    ...
    expect(parseRoute(`/envios?cliente=${encodeURIComponent('Ana María')}`)).toEqual({ view: 'shipments', guia: null, cliente: 'Ana María', unassigned: false })
    expect(parseRoute('/envios')).toEqual({ view: 'shipments', guia: null, cliente: null, unassigned: false })
  })

  it('carries an unassigned=1 query param', () => {
    expect(pathFor('shipments', null, null, true)).toBe('/envios?unassigned=1')
    expect(parseRoute('/envios?unassigned=1')).toEqual({ view: 'shipments', guia: null, cliente: null, unassigned: true })
    expect(parseRoute('/envios?cliente=X&unassigned=1')).toEqual({ view: 'shipments', guia: null, cliente: 'X', unassigned: true })
  })
  ...
  it('encodes the detail guia into /envio/:guia and back', () => {
    ...
    expect(parseRoute(`/envio/${encodeURIComponent(guia)}`)).toEqual({ view: 'shipments', guia, cliente: null, unassigned: false })
  })

  it('parses unknown paths as overview', () => {
    expect(parseRoute('/whatever')).toEqual({ view: 'overview', guia: null, cliente: null, unassigned: false })
    expect(parseRoute('/')).toEqual({ view: 'overview', guia: null, cliente: null, unassigned: false })
  })

  it('falls back to overview on invalid percent-encoding in /envio/:guia', () => {
    expect(parseRoute('/envio/%zz')).toEqual({ view: 'overview', guia: null, cliente: null, unassigned: false })
    expect(parseRoute('/envio/%E0%A4%A')).toEqual({ view: 'overview', guia: null, cliente: null, unassigned: false })
  })
